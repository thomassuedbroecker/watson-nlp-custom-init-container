...
function verifyWatsonNLPContainer () {
    ...
    RESULT=$(kubectl exec --stdin --tty $POD --container $FIND -- curl -X POST "http://localhost:8080/v1/watson.runtime.nlp.v0/NlpService/ClassificationPredict" -H "accept: application/json" -H "grpc-metadata-mm-model-id: custom_model" -H "content-type: application/json" -d '{ "rawDocument": { "text": "The credit card does not work, and I look at the savings, but I need more money to spend." }}')
    echo ""
...
function verifyWatsonNLPLoadbalancer () {
    ...
    curl -X POST "http://$EXTERNAL_IP:8080/v1/watson.runtime.nlp.v0/NlpService/ClassificationPredict" -H "accept: application/json" -H "grpc-metadata-mm-model-id: watson-nlp_custom_model:1.0.0" -H "content-type: application/json" -d '{ "rawDocument": { "text": "The credit card does not work, and I look at the savings, but I need more money to spend." }}'
}
